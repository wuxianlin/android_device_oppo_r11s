...
public class SoterKeyStoreKeyPairRSAGeneratorSpi extends KeyPairGeneratorSpi {
    ...
    private void addAlgorithmSpecificParameters(KeymasterArguments keymasterArgs) {
        ...
        if (isForSoter) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_IS_FROM_SOTER);
            keymasterArgs.addUnsignedInt(SoterKeymasterDefs.KM_TAG_SOTER_UID, (long)android.os.Process.myUid());
        }
        
        if (isAutoSignedWithAttkWhenGetPublicKey) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_IS_AUTO_SIGNED_WITH_ATTK_WHEN_GET_PUBLIC_KEY);
        }
        
        if (isAutoSignedWithCommonkWhenGetPublicKey) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_IS_AUTO_SIGNED_WITH_COMMON_KEY_WHEN_GET_PUBLIC_KEY);
            ...
            keymasterArgs.addBytes(SoterKeymasterDefs.KM_TAG_SOTER_AUTO_SIGNED_COMMON_KEY_WHEN_GET_PUBLIC_KEY, (Credentials.USER_PRIVATE_KEY + mAutoSignedKeyNameWhenGetPublicKey).getBytes());
			}
        }
        
        if (isAutoAddCounterWhenGetPublicKey) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_AUTO_ADD_COUNTER_WHEN_GET_PUBLIC_KEY);
        }
        
        
        if (isSecmsgFidCounterSignedWhenSign) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_IS_SECMSG_FID_COUNTER_SIGNED_WHEN_SIGN);
        }
        
        if (isNeedNextAttk) {
            keymasterArgs.addBoolean(SoterKeymasterDefs.KM_TAG_SOTER_USE_NEXT_ATTK);
        }
